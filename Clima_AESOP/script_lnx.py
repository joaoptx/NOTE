...
PASTA_MONITORADA = "/home/cendas/ads"
PASTA_SAIDA = "/home/cendas/storage/aesop_clima_ts" 
SHAPE_MUNICIPIOS = "/home/cendas/ads/shapefile/BR_Municipios_2024/BR_Municipios_2024.shp"

INICIO_SEMANA_UTC = (6, 10)  # Domingo 10h UTC (7h BRT)
...
def salvar_semana_csv(df_semana, data_inicio, data_fim):
    ...
    semana_str = f"{data_inicio.strftime('%d/%m/%Y')}-{data_fim.strftime('%d/%m/%Y')}"
    ...
    for _, row in df_semana.iterrows():
        try:
            ...
            nova_linha = pd.DataFrame([{
                ...
                "NM_MUN": nm_mun,
                "CD_MUN": cd_mun,
                "Temp_Media_C": round(row["temp"] - 273.15, 2) if 'temp' in row else None,  # Converter Kelvin para Celsius
                "Pressao_Media_hPa": round(row["press"] / 100, 2) if 'press' in row else None,  # Converter Pa para hPa
                "Umidade_Relativa_%": round(row["ur"], 2) if 'ur' in row else None,
                "Precipitacao_Acumulada_mm": round(row["precip"] * 1000, 2) if 'precip' in row else None  # Converter m para mm
            }])
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PASTA_MONITORADA):
        logger.error(f"Pasta monitorada não existe: {PASTA_MONITORADA}")
        exit(1)
    
    if not os.path.exists(PASTA_SAIDA):
        logger.error(f"Pasta de saída não existe: {PASTA_SAIDA}")
        exit(1)
    
    # Verificar permissões de escrita
    if not os.access(PASTA_SAIDA, os.W_OK):
        logger.error(f"Sem permissão de escrita na pasta de saída: {PASTA_SAIDA}")
        exit(1)

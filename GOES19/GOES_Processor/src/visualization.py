...
# --- IMPORTS DO PROJETO ---
from src import config, colormaps, processing, file_manager, glm_handler, utils
...
def plot_glm_overlay(ax, utc_dt):
    """
    Consulta o glm_handler e plota os raios no eixo fornecido.
    """
    try:
        lats, lons = glm_handler.get_glm_data(utc_dt)
        
        if lats is not None and lons is not None and len(lats) > 0:
            # --- CORRIGIDO: MUDADO PARA AZUL ELÉTRICO (#00BFFF) ---
            # O "Deep Sky Blue" garante visibilidade tanto no mar quanto nas nuvens.
            ax.scatter(lons, lats, s=8, c='#00BFFF', marker='+', 
                       transform=ccrs.PlateCarree(), 
                       alpha=1.0, zorder=5, label='GLM Lightning', linewidths=0.8)
            return True
        return False
    except Exception as e:
        logging.warning(f"Erro ao adicionar overlay GLM: {e}")
        return False

def process_nc_file(band_num, band_folder, nc_path):
    """
    Processa NetCDF gerando imagens limpas e, se for Banda 13, imagens com GLM.
    """
    try:
        ds = None; data = None; Lon_data = None; Lat_data = None
        
        try:
            ds = GOES.open_dataset(nc_path)
            result = ds.image('CMI', lonlat='corner', domain=config.DOMAIN)
            
            if not result or any(r is None for r in result): return
            
            CMI, Lon, Lat = result
            if CMI.data is None or np.all(np.isnan(CMI.data)): return
        ...
        path_brasil = utils.get_structure_path(utc_dt, "imagem", region="Brasil", banda=band_num)
        jpg_name = f'{ts_str}_Band{band_num:02d}_Brasil_{os.path.basename(nc_path).replace(".nc", "")}.jpg'
        jpg_path = os.path.join(path_brasil, jpg_name)
        
        fig.savefig(jpg_path, format='jpeg', bbox_inches='tight')

        if band_num == 13:
            has_lightning = plot_glm_overlay(ax, utc_dt)
            if has_lightning:
                path_glm_br = utils.get_structure_path(utc_dt, "glm", region="Brasil", banda=band_num)
                fig.savefig(os.path.join(path_glm_br, jpg_name), format='jpeg', bbox_inches='tight')
        ...
        path_sudeste = utils.get_structure_path(utc_dt, "imagem", region="Sudeste", banda=band_num)
        jpg_se_name = f'{ts_str}_Band{band_num:02d}_Sudeste_{os.path.basename(nc_path).replace(".nc", "")}.jpg'
        jpg_se_path = os.path.join(path_sudeste, jpg_se_name)
        ...
        if band_num == 13:
            has_lightning = plot_glm_overlay(ax, utc_dt)
            if has_lightning:
                path_glm_se = utils.get_structure_path(utc_dt, "glm", region="Sudeste", banda=band_num)
                fig.savefig(os.path.join(path_glm_se, jpg_se_name), format='jpeg', bbox_inches='tight')
        ...
        gc.collect() 
        recorte_dir = utils.get_structure_path(utc_dt, "netcdf", banda=band_num)
